...
bcftools view -S selected-sample.txt $PROJDIR/combined.jc.vcf.gz > $PROJDIR/$SUBSET/combined.jc.ss.vcf

# compress the selected-samples vcf 
bcftools view $PROJDIR/$SUBSET/combined.jc.ss.vcf -Oz -o $PROJDIR/$SUBSET/combined.jc.ss.vcf.gz
bcftools index $PROJDIR/$SUBSET/combined.jc.ss.vcf.gz
STEP1
...
bcftools view $PROJDIR/$SUBSET/combined.jc.ss.vcf.gz -q 0.03:minor -R $PROJDIR/$SUBSET/genes-selected.bed -o $PROJDIR/$SUBSET/combined.jc.ss.gs.vcf
#STEP2

#<<'STEP4'
#step 4. bzip and index the .vcf containing the genes selected
bcftools view $PROJDIR/$SUBSET/combined.jc.ss.gs.vcf -q 0.03:minor -Oz -o $PROJDIR/$SUBSET/combined.jc.ss.gs.vcf.gz
bcftools index $PROJDIR/$SUBSET/combined.jc.ss.gs.vcf.gz
#STEP4

#<<'STEP5'
# step 5. Predict the effects of variants using Ensemble Variant Effect Predictor
singularity exec $PROJDIR/tools/vep.sif vep --fork 4 --dir $PROJDIR/tools/vep_data \
    --max_af -i $PROJDIR/$SUBSET/combined.jc.ss.gs.vcf --format vcf \
    --force_overwrite -o $PROJDIR/$SUBSET/combined.jc.ss.gs.vep.vcf --vcf --cache
#STEP5

# Lets filter only most-severe-consequence-variants, see consequence-list to filter in filter-variants.txt
conseq="_mscvo" 
bcftools view --header $PROJDIR/$SUBSET/combined.jc.ss.gs.vep.vcf > $PROJDIR/$SUBSET/combined.jc.ss.gs.vep${conseq}.vcf
egrep -f $PROJDIR/$SUBSET/filter-variants.txt $PROJDIR/$SUBSET/combined.jc.ss.gs.vep.vcf >> $PROJDIR/$SUBSET/combined.jc.ss.gs.vep${conseq}.vcf
# Lets check the correctness of the filtered vcf file
bcftools view $PROJDIR/$SUBSET/combined.jc.ss.gs.vep${conseq}.vcf -Oz $PROJDIR/$SUBSET/combined.jc.ss.gs.vep${conseq}.vcf.gz
#<<'STEP6'
...
$PROJDIR/tools/plink_linux_x86_64_20231211/plink --noweb --const-fid 0 --maf 0.03 --keep-allele-order \
    --vcf $PROJDIR/$SUBSET/combined.jc.ss.gs.vep.vcf \
    --recode --out $PROJDIR/$SUBSET/combined.jc.ss.gs.vep.plink
#STEP6
...
input_FILT_VCF="combined.jc.ss.gs.vep${conseq}.vcf"
input_PHENO_FILT="sample-phenotypes-plink.txt"
params_BFILE="combined.jc.ss.gs.vep${conseq}.qc"
# 
