...
bcftools view $PROJDIR/$SUBSET/combined.jc.ss.vcf.gz -q 0.03:minor -R $PROJDIR/$SUBSET/genes-selected.bed -o $PROJDIR/$SUBSET/combined.jc.ss.vcf
STEP2

#<<'STEP4'
#step 4. bzip and index the .vcf containing the genes selected
bcftools view $PROJDIR/$SUBSET/combined.jc.ss.vcf -q 0.03:minor -Oz -o $PROJDIR/$SUBSET/combined.jc.ss.vcf.gz
bcftools index $PROJDIR/$SUBSET/combined.jc.ss.vcf.gz
#STEP4
...
singularity exec $PROJDIR/tools/vep.sif vep --fork 16 --dir $PROJDIR/tools/vep_data \
    --max_af -i $PROJDIR/$SUBSET/combined.jc.ss.vcf --format vcf \
    --force_overwrite -o $PROJDIR/$SUBSET/combined.jc.ss.vep.vcf --vcf --cache
#STEP5

# Lets filter only most-severe-consequence-variants, see consequence-list to filter in filter-variants.txt
conseq="_mscvo" 
bcftools view --header $PROJDIR/$SUBSET/combined.jc.ss.vep.vcf > $PROJDIR/$SUBSET/combined.jc.ss.vep${conseq}.vcf
egrep -f $PROJDIR/$SUBSET/filter-variants.txt $PROJDIR/$SUBSET/combined.jc.ss.vep.vcf >> $PROJDIR/$SUBSET/combined.jc.ss.vep${conseq}.vcf
# Lets check the correctness of the filtered vcf file
bcftools view $PROJDIR/$SUBSET/combined.jc.ss.vep${conseq}.vcf -Oz $PROJDIR/$SUBSET/combined.jc.ss.vep${conseq}.vcf.gz
#<<'STEP6'
...
$PROJDIR/tools/plink_linux_x86_64_20231211/plink --noweb --const-fid 0 --maf 0.03 --keep-allele-order \
    --vcf $PROJDIR/$SUBSET/combined.jc.ss.vep.vcf \
    --recode --out $PROJDIR/$SUBSET/combined.jc.ss.vep.plink
#STEP6
...
input_FILT_VCF="combined.jc.ss.vep${conseq}.vcf"
input_PHENO_FILT="sample-phenotypes-plink.txt"
params_BFILE="combined.jc.ss.vep${conseq}.qc"
# 
